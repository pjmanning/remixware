 ...
 */
const host = process.env.VITE_SITE_URL?.trim() || 'http://localhost:3000'

/** Routes behind auth are never prerendered or listed in the sitemap. */
const isPrivatePath = (path: string) => path.startsWith('/app') || path.startsWith('/login')
 ...
 */
const privatePages = ['/app', '/app/', '/app/settings', '/login', '/login?mode=sign-up'].map(
  (path) => ({
    path,
 ...
 */
const duplicateIndexPages = ['/blog/', '/catalog/'].map((path) => ({
  path,
  sitemap: { exclude: true },
}))
